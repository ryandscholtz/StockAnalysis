...
def get_ticker_data(ticker: str) -> dict:
    """Get stock ticker data - tries yfinance first, falls back to MarketStack"""
    # Try yfinance first
    ...
    # Fall back to Yahoo Finance (no API key, no IP restrictions)
    ...
    return {
        'statusCode': 503,
        'body': json.dumps({'error': 'Stock data unavailable', 'details': yf_data.get('error', '')})
    }
