...
class TestIntegrationWorkflows:
    ...
    def test_complete_pdf_processing_workflow(self):
        ...
        upload_response = self.client.post(
            f"/api/upload-pdf?ticker={test_ticker}", files=files)
        ...
        if upload_response.status_code == 200:
            ...
            # Processing might fail due to test PDF content, but we should get proper
            # status
            ...
            pytest.fail(
                f"Unexpected upload response: {upload_response.status_code} - {upload_response.text}")
