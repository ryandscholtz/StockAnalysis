...
def make_request(client: TestClient, endpoint: str, method: str = "GET", **kwargs) -> Dict[str, Any]:
...
def run_concurrent_requests(endpoint: str, num_requests: int, num_workers: int = 10, method: str = "GET", **kwargs) -> LoadTestResults:
...
class TestLoadPerformance:
    ...
    def test_system_handles_expected_load_light(self):
        ...
        for endpoint in endpoints:
            ...
            assert results.success_rate >= 95.0, f"Endpoint {endpoint} failed light load test: {results.success_rate}% success rate"
        
        # Overall system performance
        avg_success_rate = sum(r.success_rate for r in all_results) / len(all_results)
        assert avg_success_rate >= 95.0, f"Overall system success rate {avg_success_rate:.1f}% below 95%"
    ...
    def test_system_handles_expected_load_medium(self):
        ...
        assert results.success_rate >= 95.0, f"Medium load test failed: {results.success_rate}% success rate"
        assert results.avg_response_time < 0.2, f"Medium load response time {results.avg_response_time:.3f}s too slow"
        assert results.requests_per_second >= 50, f"Medium load throughput {results.requests_per_second:.1f} RPS too low"
    ...
    def test_system_handles_expected_load_heavy(self):
        ...
        results = run_concurrent_requests("/api/version", num_requests=150, num_workers=50)
        
        # System should handle heavy load with some degradation acceptable
        assert results.success_rate >= 90.0, f"Heavy load test failed: {results.success_rate}% success rate"
        assert results.avg_response_time < 1.0, f"Heavy load response time {results.avg_response_time:.3f}s too slow"
        assert results.p99_response_time < 5.0, f"Heavy load 99th percentile {results.p99_response_time:.3f}s too slow"
    ...
    def test_health_endpoint_load(self):
        ...
        # Assertions
        assert results.success_rate >= 99.0, f"Success rate {results.success_rate}% below 99%"
        assert results.avg_response_time < 0.1, f"Average response time {results.avg_response_time:.3f}s exceeds 100ms"
        assert results.p95_response_time < 0.2, f"95th percentile response time {results.p95_response_time:.3f}s exceeds 200ms"
    ...
    def test_version_endpoint_load(self):
        ...
        Requirements: 5.6 - Performance SLA compliance
        """
        results = run_concurrent_requests("/api/version", num_requests=50, num_workers=10)
    ...
    def test_search_endpoint_load(self):
        ...
        Requirements: 5.6 - Performance SLA compliance
        """
        search_queries = ["AAPL", "MSFT", "GOOGL", "TSLA", "AMZN", "META", "NVDA", "BRK", "JPM", "V"]
        
        # Test each query with moderate concurrency
        for query in search_queries[:3]:  # Test first 3 to keep test time reasonable
            results = run_concurrent_requests(f"/api/search?q={query}", num_requests=20, num_workers=5)
            
            # Assertions
            assert results.success_rate >= 95.0, f"Search '{query}' success rate {results.success_rate}% below 95%"
            assert results.avg_response_time < 5.0, f"Search '{query}' average response time {results.avg_response_time:.3f}s exceeds 5s"
    ...
    def test_cached_quote_endpoint_performance(self):
        ...
        for ticker in tickers:
            results = run_concurrent_requests(f"/api/quote/{ticker}/cached", num_requests=30, num_workers=10)
            
            # For cached data, we expect sub-200ms response times
            assert results.avg_response_time < 0.2, f"Cached quote {ticker} average response time {results.avg_response_time:.3f}s exceeds 200ms"
            assert results.p95_response_time < 0.5, f"Cached quote {ticker} 95th percentile {results.p95_response_time:.3f}s exceeds 500ms"
    ...
    def test_concurrent_user_scenarios(self):
        ...
        with TestClient(app) as client:
            with ThreadPoolExecutor(max_workers=15) as executor:
                ...
                for future, endpoint in futures:
                    ...
                    endpoint_results[endpoint].response_times.append(result["response_time"])
                    endpoint_results[endpoint].status_codes.append(result["status_code"])
                    if result["error"]:
        ...
        for endpoint, results in endpoint_results.items():
            ...
            assert results.success_rate >= 95.0, f"Endpoint {endpoint} success rate {results.success_rate}% below 95%"
            
            # Performance requirements vary by endpoint type
            if "/health" in endpoint or "/version" in endpoint:
                assert results.avg_response_time < 0.1, f"Fast endpoint {endpoint} avg time {results.avg_response_time:.3f}s exceeds 100ms"
            elif "/search" in endpoint:
                assert results.avg_response_time < 5.0, f"API endpoint {endpoint} avg time {results.avg_response_time:.3f}s exceeds 5s"
    ...
    def test_sustained_load_performance(self):
        ...
        assert results.success_rate >= 98.0, f"Sustained load success rate {results.success_rate}% below 98%"
        assert results.avg_response_time < 0.5, f"Sustained load avg response time {results.avg_response_time:.3f}s exceeds 500ms"
        assert len(results.errors) <= total_requests * 0.02, f"Too many errors: {len(results.errors)}/{total_requests}"
    ...
    def test_error_handling_under_load(self):
        ...
        for endpoint in invalid_endpoints:
            ...
            assert results.avg_response_time < 1.0, f"Error response time {results.avg_response_time:.3f}s too slow for {endpoint}"
            
            # Should return proper HTTP error codes (not 500 for client errors)
            if "/nonexistent" in endpoint:
                assert all(code == 404 for code in results.status_codes), f"Expected 404s for {endpoint}"
            elif "/search" in endpoint and "?" not in endpoint:
                assert all(code == 422 for code in results.status_codes), f"Expected 422s for {endpoint}"
...
class TestMemoryAndResourceUsage:
    ...
    def test_memory_usage_stability(self):
        ...
        assert memory_increase < 50, f"Memory increased by {memory_increase:.1f}MB, may indicate memory leak"
        assert results.success_rate >= 99.0, f"Load test failed with {results.success_rate}% success rate"
