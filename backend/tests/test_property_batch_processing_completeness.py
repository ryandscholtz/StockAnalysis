...
def invalid_record(draw):
    ...
    record_type = draw(st.sampled_from(['missing_ticker', 'negative_price', 'zero_volume', 'invalid_timestamp']))
    ...
    if record_type == 'missing_ticker':
        ...
        return {
            ...
            "price": draw(st.floats(max_value=-0.01, allow_nan=False, allow_infinity=False)),
            "volume": draw(st.integers(min_value=1, max_value=1000000))
        }
...
def mixed_dataset(draw):
    ...
    valid_records = draw(st.lists(valid_record(), min_size=valid_count, max_size=valid_count))
    invalid_records = draw(st.lists(invalid_record(), min_size=invalid_count, max_size=invalid_count))
...
class TestBatchProcessingCompleteness:
    ...
    def mock_data_quality_service(self):
        ...
        async def mock_validate(data: Dict[str, Any], data_type: str = "stock_data") -> DataQualityReport:
    ...
    async def test_batch_processing_completeness_property(self, batch_service, data_tuple):
        """
        ...
        **Validates: Requirements 10.4**
        ...
        assert result.total_records == len(dataset), "Total records should match input dataset size"
        
        # All records should be processed (either successfully or with logged failures)
        processed_count = result.successful_records + result.failed_records + result.skipped_records
        ...
        assert len(failed_records) == result.failed_records, "Failed record count should match"
        
        for record in failed_records:
            assert record.error_message is not None, f"Failed record {record.id} should have error message"
            assert record.processed_at is not None, f"Failed record {record.id} should have processed timestamp"
        
        # All successful records should have results
        successful_records = [r for r in result.records if r.status == RecordStatus.SUCCESS]
        assert len(successful_records) == result.successful_records, "Successful record count should match"
        
        for record in successful_records:
            assert record.result is not None, f"Successful record {record.id} should have result"
            assert record.processed_at is not None, f"Successful record {record.id} should have processed timestamp"
        ...
        if result.failed_records > 0:
            assert len(result.error_summary) > 0, "Error summary should contain error messages for failed records"
            ...
            assert failed_record_ids.issubset(summary_record_ids), "All failed records should be in error summary"
    
    @given(st.lists(valid_record(), min_size=1, max_size=20))
    @pytest.mark.asyncio
    async def test_all_valid_records_processed_successfully(self, batch_service, valid_dataset):
        """
    ...
    @pytest.mark.asyncio
    async def test_all_invalid_records_logged_with_reasons(self, batch_service, invalid_dataset):
        """
    ...
    async def test_processor_failures_are_logged(self, batch_service, dataset):
        ...
        processor_failed_records = [
            ...
            if r.status == RecordStatus.FAILED and "Processing error" in (r.error_message or "")
        ]
    ...
    async def test_batch_job_metadata_completeness(self, batch_service, dataset):
        ...
        for record in result.records:
            ...
            assert record.processed_at is not None
            assert record.status in [RecordStatus.SUCCESS, RecordStatus.FAILED, RecordStatus.SKIPPED]