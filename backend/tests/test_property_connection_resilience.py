...
from app.database.connection import (
    RetryPolicy, SimpleConnectionPool, ResilientDatabaseService, 
    with_retry, ConnectionError
)
...
class FailingRepository:
    ...
    async def save_analysis(self, ticker: str, analysis_data, exchange=None, analysis_date=None):
        self.call_count += 1
        if self.failure_count < self.max_failures and self.call_count <= self.max_failures:
            self.failure_count += 1
            raise ConnectionError(f"Simulated connection failure {self.failure_count}")
...
class TestConnectionResilience:
    ...
    async def test_retry_policy_resilience_property(self, max_attempts, base_delay, max_failures):
        """
        ...
        **Validates: Requirements 2.6, 5.4, 10.1**
        ...
        try:
            ...
            assert failing_repo.call_count == expected_calls, f"Expected {expected_calls} calls, got {failing_repo.call_count}"
            ...
            assert failing_repo.call_count == max_attempts, f"Expected {max_attempts} calls, got {failing_repo.call_count}"
    ...
    async def test_connection_pool_resilience_property(self, pool_size, concurrent_operations):
        """
        Feature: tech-stack-modernization, Property 5: Connection Resilience
        ...
        **Validates: Requirements 2.6, 5.4, 10.1**
        ...
        try:
            ...
            assert len(connections) <= pool_size, f"Should not exceed pool size {pool_size}"
    ...
    @settings(max_examples=15, deadline=5000)
    async def test_service_resilience_statistics_property(self, operation_count, failure_probability):
        """
        Feature: tech-stack-modernization, Property 5: Connection Resilience
        ...
        **Validates: Requirements 2.6, 5.4, 10.1**
        ...
        assert stats["operation_count"] == operation_count, f"Expected {operation_count} operations, got {stats['operation_count']}"
        
        # Property: Failure count should match actual failures
        assert stats["failure_count"] == failed_operations, f"Expected {failed_operations} failures, got {stats['failure_count']}"
        
        # Property: Success rate should be calculated correctly
        expected_success_rate = successful_operations / operation_count
        assert abs(stats["success_rate"] - expected_success_rate) < 0.01, f"Success rate mismatch: expected {expected_success_rate}, got {stats['success_rate']}"
    ...
    async def test_retry_delay_calculation_property(self, exponential_backoff, jitter, max_attempts):
        """
        Feature: tech-stack-modernization, Property 5: Connection Resilience
        ...
        **Validates: Requirements 2.6, 5.4, 10.1**
        ...
        assert all(d <= max_delay for d in delays), f"All delays should be <= {max_delay}"
        ...
        if exponential_backoff and not jitter and len(delays) > 1:
            ...
            for i in range(1, len(delays)):
                if delays[i-1] < max_delay:  # Only check if previous delay wasn't capped
                    ...
                    assert delays[i] >= delays[i-1] * 0.9, f"Exponential backoff should increase delays: {delays}"
        
        # Property: If exponential backoff is disabled, delays should be consistent (ignoring jitter)
        if not exponential_backoff and not jitter:
            expected_delay = min(base_delay, max_delay)
            for delay in delays:
                assert abs(delay - expected_delay) < 0.001, f"Fixed delay should be consistent: expected {expected_delay}, got {delay}"
    ...
    async def test_connection_pool_cleanup_property(self):
        ...
        **Validates: Requirements 2.6, 5.4, 10.1**
        ...
        assert len(closed_connections) == len(created_connections), "All created connections should be closed"
