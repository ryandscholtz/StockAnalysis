            ...
            st.text(
                min_size=1, max_size=50, alphabet=st.characters(
                    whitelist_categories=(
                        'Lu', 'Ll', 'Nd', 'Pc')))), "source_type": draw(
            st.sampled_from(source_types)), "location": draw(
                            st.one_of(
                                st.none(), st.text(
                                    min_size=1, max_size=100))), "metadata": draw(
                                        st.dictionaries(
                                            st.text(
                                                min_size=1, max_size=20), st.one_of(
                                                    st.text(), st.integers(), st.floats(
                                                        allow_nan=False, allow_infinity=False)), max_size=5))}
...
class TestDataLineageTraceability:
    ...
    def test_data_lineage_traceability_property(self, chain_data):
        ...
        **Validates: Requirements 10.5**
        ...
        def contains_source_in_lineage(
                lineage_data: Dict[str, Any], target_source_id: str) -> bool:
        ...
        transformation_path = lineage_service.get_transformation_path(
            original_source_id, final_source_id)
        assert len(
            transformation_path) >= 2, "Should have a path from original to final source"
    ...
    def test_transformation_bidirectional_traceability(
            self, input_source_info, output_source_info, transform_info):
        """
        ...
        assert input_id in [upstream["source"]["id"] for upstream in upstream_lineage.get(
            "upstream", {}).values()], "Input source should be traceable upstream from output"

        # Test downstream tracing from input
        downstream_lineage = lineage_service.trace_lineage_downstream(input_id)
        assert output_id in [downstream["source"]["id"] for downstream in downstream_lineage.get(
            "downstream", {}).values()], "Output source should be traceable downstream from input"

        # Transformation should be recorded in both directions
        assert len(upstream_lineage.get("transformations", [])
                   ) > 0, "Output should have transformation recorded"
    ...
    def test_complex_lineage_traceability(self, source_infos):
        ...
        for i, derived_source_id in enumerate(source_ids[1:], 1):
            ...
            def find_source_in_lineage(
                    lineage_data: Dict[str, Any], target_id: str) -> bool:
    ...
    def test_lineage_integrity_validation(self, num_sources):
        ...
        for i in range(min(num_sources - 1, 5)
                       ):  # Limit transformations to avoid complexity
        ...
        assert len(
            integrity_issues) == 0, f"Valid lineage should have no integrity issues, but found: {integrity_issues}"
