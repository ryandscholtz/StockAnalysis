...
class TestDatabaseMigrationIdempotency:
        ...
        **Validates: Requirements 2.5**
        ...
        try:
            ...
            for migration in migrations:
                assert migration.up_call_count == 1, f"Migration {
                    migration.version} up() called {
                    migration.up_call_count} times, expected 1"
            ...
            for attempt in range(3):
                try:
                    ...
                    if attempt < 2:  # Retry up to 3 times
                        ...
                        # Log warning but don't fail test - temp files will be cleaned
                        # up eventually
                        import warnings
                        warnings.warn(
                            f"Could not delete temporary file {tmp_path} - will be cleaned up by OS")
    ...
    async def test_partial_migration_idempotency_property(self, migration_versions):
        ...
        **Validates: Requirements 2.5**
        ...
        tmp_path = os.path.join(
            tmp_dir, f"test_partial_migration_{
                uuid.uuid4().hex}.db")
        ...
        try:
            ...
            if len(migrations) > 1:
                first_migration = migrations[0]
                assert first_migration.up_call_count == 0, f"Already applied migration {
                    first_migration.version} should not be called again"

            # Property: Remaining migrations should be applied exactly once
            for migration in migrations[1:]:
                assert migration.up_call_count == 1, f"Migration {
                    migration.version} should be called exactly once"
    ...
    @pytest.mark.asyncio
    @given(
        migration_version=st.text(
            min_size=1, max_size=10, alphabet=st.characters(
                whitelist_categories=(
                    'Nd', 'Lu', 'Ll'))), rollback_count=st.integers(
            min_value=2, max_value=4))
    @settings(max_examples=30, deadline=5000)
    async def test_rollback_idempotency_property(
            self, migration_version, rollback_count):
        """
        Feature: tech-stack-modernization, Property 4: Database Migration Idempotency
        ...
        **Validates: Requirements 2.5**
        ...
        try:
            ...
            # Property: First rollback should succeed, subsequent ones should fail
            # gracefully
            assert rollback_results[0], "First rollback should succeed"
            ...
            assert migration.down_call_count == 1, f"Migration down() called {
                migration.down_call_count} times, expected 1"
    ...
    async def test_migration_failure_handling_property(self):
        ...
        **Validates: Requirements 2.5**
        ...
        try:
            ...
            result = await runner.run_migrations()
            assert result, "Retry after fixing should succeed"
