...
class TestHTTPSCommunicationEnforcement:
                    ...
                    host=st.text(
                        ...
                        alphabet=st.characters(
                            whitelist_categories=(
                                ...
                                'Nd'))),
                    path=st.text(
                        min_size=0, max_size=50, alphabet=st.characters(
                            whitelist_categories=(
                                'Lu', 'Ll', 'Nd', 'Pc')))
    ...
    def test_https_communication_enforcement_property(
            self, urls: List[str], enforce_https: bool, redirect_http: bool):
        """
        ...
        for url in urls:
            ...
            if parsed_url.scheme == "https":
                ...
                if enforce_https:
                    ...
                    # But security headers should still be present if response is
                    # successful
                    if response["status_code"] == 200:
                        assert "X-Content-Type-Options" in response.get(
                            "headers", {}), f"Even HTTP responses should include security headers for {url}"
    ...
    def test_https_security_headers_property(
            self, base_urls: List[str], endpoints: List[str]):
        """
    ...
    def test_tls_configuration_security_property(
            self, tls_versions: List[str], cipher_suites: List[str]):
        """
