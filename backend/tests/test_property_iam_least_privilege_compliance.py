...
class MockIAMService:
    ...
    def create_role(self, role_name: str, service: str, permissions: List[str], resources: List[str] = None) -> Dict[str, Any]:
    ...
    def check_permission(self, role_name: str, action: str, resource: str = "*") -> Dict[str, Any]:
    ...
    def analyze_least_privilege(self, role_name: str, required_actions: List[str]) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "missing_permissions": list(required_permissions - role_permissions)
        }
        
        return analysis
    
    def test_unauthorized_access(self, role_name: str, unauthorized_actions: List[str]) -> Dict[str, Any]:
...
class TestIAMLeastPrivilegeCompliance:
    ...
    @given(
        role_configs=st.lists(
            st.tuples(
                st.text(min_size=3, max_size=20, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'Pc'))),  # role_name
    ...
    def test_iam_least_privilege_compliance_property(self, role_configs: List[tuple], required_actions: List[str]):
        """
        ...
        **Validates: Requirements 7.4**
        ...
        for role_name, service, permissions in role_configs:
            ...
            assert len(analysis["admin_permissions"]) == 0, \
                f"Role {role_name} should not have administrative permissions: {analysis['admin_permissions']}"
    ...
    @given(
        service_roles=st.lists(
            st.tuples(
                st.sampled_from(["stock-analysis-api", "pdf-processor", "data-fetcher", "cache-manager"]),  # service_name
                st.lists(
                    st.sampled_from([
    ...
    def test_service_specific_permissions_property(self, service_roles: List[tuple]):
        ...
        **Validates: Requirements 7.4**
        ...
        for service_name, permissions, resources in service_roles:
            ...
            # Verify role doesn't have wildcard resources when specific ones are provided
            analysis = iam_service.analyze_least_privilege(role_name, permissions)
            if len(resources) > 0 and "*" not in resources:
                # Role should not have wildcard access when specific resources are provided
            ...
            for resource in resources:
                for permission in permissions:
                    check_result = iam_service.check_permission(role_name, permission, resource)
                    assert check_result["allowed"] is True, \
    ...
    @given(
        roles_and_unauthorized_actions=st.lists(
            st.tuples(
                st.text(min_size=3, max_size=15, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'))),  # role_name
                st.lists(
                    st.sampled_from([
    ...
    def test_unauthorized_access_prevention_property(self, roles_and_unauthorized_actions: List[tuple]):
        """
        ...
        **Validates: Requirements 7.4**
        ...
        for role_name, granted_permissions, unauthorized_actions in roles_and_unauthorized_actions:
            ...
            role = iam_service.create_role(role_name, "lambda", granted_permissions)
            
            # Test unauthorized access
            unauthorized_test = iam_service.test_unauthorized_access(role_name, unauthorized_actions)
    ...
    @given(
        cross_service_scenarios=st.lists(
            st.tuples(
                st.sampled_from(["api-service", "data-processor", "auth-service"]),  # service_type
                st.lists(
                    st.sampled_from([
    ...
    def test_cross_service_access_control_property(self, cross_service_scenarios: List[tuple]):
        """
        ...
        **Validates: Requirements 7.4**
        ...
        for service_type, required_access, prohibited_access in cross_service_scenarios:
            ...
            required_resource_types = [access[1] for access in required_access]
            
            prohibited_permissions = [access[0] for access in prohibited_access]
            
            # Create role with only required permissions
            role = iam_service.create_role(role_name, "lambda", required_permissions)
            ...
            for prohibited_permission in prohibited_permissions:
                check_result = iam_service.check_permission(role_name, prohibited_permission)
            ...
            all_permissions = required_permissions + prohibited_permissions
            analysis = iam_service.analyze_least_privilege(role_name, required_permissions)
            
            # Should not have excessive permissions beyond what's required
            excessive_perms = set(analysis["granted_permissions"]) - set(required_permissions)
