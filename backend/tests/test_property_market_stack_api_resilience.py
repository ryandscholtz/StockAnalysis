...
class TestMarketStackAPIResilience:
    ...
    @given(
        ...
        api_response_time_ms=st.integers(min_value=50, max_value=5000),
        market_stack_price=st.floats(min_value=0.01, max_value=10000.0, allow_nan=False, allow_infinity=False),
        market_stack_volume=st.integers(min_value=1000, max_value=100000000),
    ...
    def test_market_stack_api_resilience_property(
        ...
        **Validates: Requirements 10.7, 10.8**
        ...
        market_stack_client = MarketStackClient(api_key='test_key' if market_stack_available else None)
        ...
        # Property 4: Response time should be within acceptable limits when API is available
    ...
    @given(
        tickers=st.lists(
            st.text(min_size=1, max_size=10).filter(lambda x: x.strip() and x.isalnum()),
            min_size=1,
            max_size=10
        ),
        market_stack_success_rate=st.floats(min_value=0.0, max_value=1.0),
    ...
    def test_data_fetcher_market_stack_integration_property(
        ...
        **Validates: Requirements 10.7, 10.8**
            ...
            with patch.object(backup_fetcher, 'alpha_vantage_client') as mock_alpha_vantage:
                with patch.object(mock_data_fetcher, 'yahoo_client', create=True) as mock_yahoo_client:
                    ...
                    if not market_stack_fails:
                        ...
                        # The actual result depends on Google Finance, so we just verify Market Stack was tried
                        pass
        ...
        assert isinstance(mock_data_fetcher.backup_fetcher.marketstack_client, MarketStackClient)
