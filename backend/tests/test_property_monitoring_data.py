...
class TestMonitoringDataAvailability:
    ...
    @given(
        operation_type=st.sampled_from(["api_request", "database_query", "external_api_call", "cache_operation"]),
        endpoint_path=st.sampled_from(["/health", "/", "/metrics"]),
        user_id=st.text(min_size=1, max_size=20, alphabet=st.characters(min_codepoint=48, max_codepoint=122))
    )
    @settings(max_examples=30, deadline=8000, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_monitoring_data_availability_property(self, operation_type: str, endpoint_path: str, user_id: str):
        """
        ...
        **Validates: Requirements 4.4, 9.1, 9.3**
        ...
        try:
            ...
            clean_user_id = ''.join(c for c in user_id if c.isprintable() and c not in '\r\n\t').strip()
        ...
        try:
            with patch.object(app_logger, 'handle', side_effect=capture_log):
                ...
                if error_type == "not_found":
                    ...
                    expected_status = [404, 422]  # Might be 404 if endpoint doesn't exist
                else:  # internal_error
                ...
                if response.status_code >= 400:
                    ...
                    # The exact log format depends on implementation, so we check for any logs
                    ...
                    if captured_logs:
                        # Look for any logs that might contain request or error information
                        relevant_logs = [log for log in captured_logs 
                                       if any(keyword in log.get('message', '').lower() 
                                             for keyword in ['request', 'error', 'http', 'response'])]
                        
                        # This is a softer check - we expect some relevant logging but don't require specific format
                        # The main requirement is that monitoring data is available, which we verify through headers
    ...
    def test_concurrent_monitoring_property(self, concurrent_requests: int):
        ...
        **Validates: Requirements 4.4, 9.1, 9.3**
        ...
        try:
            ...
            assert len(correlation_ids) == concurrent_requests, \
                f"Each request should have unique correlation ID, got {len(correlation_ids)} unique IDs for {concurrent_requests} requests"
