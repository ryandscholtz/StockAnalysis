...
class TestRateLimitingEnforcement:
    ...
    def test_rate_limiting_enforcement_property(self, client, request_count: int, endpoint_path: str):
        """
        ...
        **Validates: Requirements 4.3, 7.3**
        ...
        if request_count <= burst_limit:
            ...
            # or that all requests are consistently rate limited (which is also valid behavior)
            status_codes = [r['status_code'] for r in responses]
            if len(successful_requests) == 0 and len(rate_limited_requests) == request_count:
                # All requests were rate limited - this is valid if previous tests exhausted the limit
                ...
                assert len(successful_requests) >= 1, \
                    f"Expected at least 1 successful request for small request count, got {len(successful_requests)}. Status codes: {status_codes}"
        else:
            # For larger request counts, we should definitely see some rate limiting
            # The key property is that rate limiting is enforced - we should see 429 responses
            status_codes = [r['status_code'] for r in responses]
    ...
    def test_concurrent_rate_limiting_property(self, concurrent_users: int, requests_per_user: int):
        """
        ...
        **Validates: Requirements 4.3, 7.3**
        ...
        async def make_user_requests(user_id: int, client: TestClient):
            ...
            for i in range(requests_per_user):
                try:
                    ...
                    headers = {"X-User-ID": str(user_id), "X-Forwarded-For": f"192.168.1.{user_id}"}
        ...
        assert len(users_with_successful_requests) >= min(concurrent_users, 3), \
            f"Expected at least some users to have successful requests, got {len(users_with_successful_requests)} users"
