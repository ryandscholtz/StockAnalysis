...
from app.core.compression import CompressionService, get_compression_service
...
class TestResponseCompressionProperty:
    ...
    def test_response_compression_efficiency_property(self, response_size: int, data_type: str):
        """
        ...
        **Validates: Requirements 5.3**
        ...
        assert result["compressed"] is True, \
            f"Response of size {actual_size} should be compressed (minimum: {compression_service.minimum_size})"
        
        # Property: Compressed size should be smaller than original
        assert result["compressed_size"] < result["original_size"], \
            f"Compressed size {result['compressed_size']} should be less than original {result['original_size']}"
    ...
    def test_small_response_not_compressed_property(self, target_size: int, content_type: str):
        """
        ...
        **Validates: Requirements 5.3**
        ...
        # Generate small content based on type, ensuring final size stays below threshold
        ...
        assert result["compressed"] is False, \
            f"Response of size {result['original_size']} should not be compressed (below minimum {compression_service.minimum_size})"
    ...
    def test_compression_level_efficiency_property(self, compression_level: int, data_size: int):
        """
    ...
    def test_media_type_compression_property(self, media_types: List[str]):
        ...
        **Validates: Requirements 5.3**
        ...
        for media_type in media_types:
            ...
            is_excluded = any(excluded_type in media_type for excluded_type in excluded_types)
            is_compressible = any(comp_type in media_type for comp_type in compressible_types)
