...
class TestSecretsManagementSecurity:
    ...
    @given(
        secret_name=st.sampled_from(["database_url", "api_key", "jwt_secret", "encryption_key"]),
        environment=st.sampled_from(["production", "staging", "development"])
    )
    @settings(max_examples=20, deadline=5000)
    def test_secrets_management_security_property(self, secret_name: str, environment: str):
        """
            ...
            with patch.dict(os.environ, {'ENVIRONMENT': environment}, clear=False):
                ...
                def get_secret_from_aws(secret_name: str) -> str:
                    ...
                    try:
                        ...
                        # Fallback to environment variable (should not happen in production)
                ...
                assert len(secret_value) > 5, f"Secret {secret_name} should have substantial length"
        ...
        **Validates: Requirements 4.5**
        ...
        with patch.dict(os.environ, {'ENVIRONMENT': 'production'}, clear=True):
            ...
            for key in config_keys:
                # In production, these should be references to secrets manager, not actual secrets
            ...
            with patch.dict(os.environ, env_vars, clear=False):
                ...
                for key in config_keys:
                    ...
                    # Verify it's a reference to secrets manager (if following AWS pattern)
                    if env_value:
    ...
    def _looks_like_secret(self, value: str) -> bool:
        ...
        secret_patterns = [
            ...
            len(value) > 32 and any(c.isdigit() for c in value) and any(c.isalpha() for c in value),  # Long mixed strings
        ]
            ...
            with patch('app.core.logging.app_logger.info', side_effect=capture_log), \
                ...
                for operation in secret_operations:
                    ...
                    try:
                        ...
                        assert isinstance(e, Exception), "Errors should be proper exceptions"
