...
class TestStreamingResponseContinuity:
    ...
    @given(
        ...
        analysis_types=st.lists(
            st.sampled_from(['dcf', 'epv', 'asset_based', 'comprehensive']),
            min_size=1,
            max_size=3
        )
    )
    ...
    def test_streaming_response_continuity_property(self, ticker, analysis_types):
        ...
        for analysis_type in analysis_types:
            ...
            assert len(progress_chunks) > 0, f"Stream should have progress updates for {analysis_type}"
            ...
            for event_type in expected_events:
                ...
                assert len(matching_chunks) > 0, f"Missing {event_type} events in stream for {analysis_type}"
    
    @given(
        tickers=st.lists(
            st.text(min_size=1, max_size=6, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'))),
            min_size=2,
            max_size=5
        )
    )
    ...
    def test_concurrent_streaming_continuity_property(self, tickers):
        ...
        for ticker, chunks in results:
            ...
            assert len(start_chunks) >= 1, f"Stream for {ticker} should have start event"
            assert len(complete_chunks) >= 1, f"Stream for {ticker} should have complete event"
            
            # Verify ticker-specific content
            ticker_chunks = [chunk for chunk in chunks if ticker in chunk]
            assert len(ticker_chunks) >= 1, f"Stream should contain ticker-specific content for {ticker}"
    
    @given(
        ticker=st.text(min_size=1, max_size=8, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'))),
        interruption_points=st.lists(
    ...
    @settings(max_examples=15, deadline=5000)
    def test_streaming_resilience_to_interruptions_property(self, ticker, interruption_points):
        """
        ...
        for interruption_point in interruption_points:
            ...
            stream = service.stream_with_interruption(ticker, fail_at=interruption_point)
            chunks = service.collect_stream_chunks(stream)
            
            assert len(chunks) > 0, f"Stream should produce some chunks before interruption at {interruption_point}"
            
            if interruption_point == 0:
                # Immediate failure
                assert any("ERROR" in chunk for chunk in chunks), "Should capture error information on immediate failure"
                ...
                assert len(successful_chunks) >= interruption_point, f"Should receive {interruption_point} chunks before failure"
                assert len(error_chunks) > 0, "Should capture error information on interruption"
            else:
                # No interruption (interruption point beyond stream length)
                error_chunks = [chunk for chunk in chunks if "ERROR" in chunk]
                assert len(error_chunks) == 0, "Should not have errors when no interruption occurs"
    
    @given(
        stream_duration=st.integers(min_value=1, max_value=10),
        chunk_interval=st.floats(min_value=0.001, max_value=0.01)  # Smaller intervals for faster tests
    )
    @settings(max_examples=10, deadline=5000)
    def test_streaming_timing_continuity_property(self, stream_duration, chunk_interval):
        """
        ...
        assert len(chunks) == stream_duration, f"Should produce {stream_duration} chunks"
        ...
        assert total_duration >= expected_duration - timing_tolerance, f"Stream too fast: {total_duration} < {expected_duration}"
        assert total_duration <= expected_duration + timing_tolerance * 2, f"Stream too slow: {total_duration} > {expected_duration}"
    ...
    def test_streaming_variable_payload_continuity_property(self, payload_sizes):
        ...
        assert len(collected_chunks) == len(payload_sizes), "Should receive all chunks regardless of size"
        
        # Verify payload integrity
        for i, (expected_size, chunk) in enumerate(zip(payload_sizes, collected_chunks)):
